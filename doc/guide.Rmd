...
-   A more logical workflow. All processes are included in a single project and data is shared across processes.
...
-   Harmonising the classification and codes used at different steps. You will use this column names all the time: **country, ref_area, NUTS, sto, activity, accounting entry, unit_measure, time_period and obs_value**.
...
-   More consistent look of the charts and reports.

-  A new predefined scatterplot to better detect outliers in the exploratory shiny app. 

-  Improve shiny reactivity to select regions.

- `{here}` is used for dealing with now more complex cases of relative folders.

- `{ggiraph}` replaces `{plotly}` for wrapping ggplot charts in javascript in most places. 
...
The script `UI.R` provides the sequential steps to be followed when analysing incoming data and the next steps after validation. It starts by loading some libraries and the user needs to type the country it is going to analyze. It will bring the xml files from the `INPUT`folder if the data has not been loaded with FameFeed or from the `DONE`folder if they have been loaded. It is preferable generally to do all these steps before loading data in Matis as it is a few seconds faster to do it from the `INPUT`folder and we will need to wait for FameFeed to finish, which can take a long time for countries with many regions. The user is also suggested to copy locally the country metadata file from the server.
...
We have prepared (draft) metadata files according to the template discussed in the 2022 EG RA meeting.
...
Not in general a very relevant issue for us as we publish the data two months later but some countries (**Greece**, **Croatia**, **Ireland**) might include a wrong date or forget to include it or it might be needed that we negotiate the publication date with them.
...
It starts by reading (again) the xml files for the country in `data/xml`. However, in this occasion we will ignore all the contextual information (flags, comments, etc) and get only the dimensions needed to identify the numbers. It takes one single value per observation. This will normally not needed as most countries provide full transmissions but occasionally we receive transmissions like this:
...
Germany corrected population for a particular year and regions (DED2, DED4) and did not provide the unchanged data for other regions. This is quite an important difference between REGACC and other domains with several implications for the use of common tools which we will no describe here.
...
We need to do some transformations in the series to create a table t1001 which takes the data up to 2020 from t1200 and only keeps the validated data from t1001 for 2021, as there might be outdated (validated two or three years ago) t1001 tables. This happens if the country usually reports only the latest year in t1001. We will need to make an exception for volume data that is only reported in the table t1001.
...
lasty<- val %>% 
  filter(table_identifier=="T1001" & time_period= 2021 & unit_measure=="XDC")# remove GVA in PYP
...
We found several cases mainly due to rounding issues for the variable self-employed which is not reported by countries but calculated by us. In such cases, we should make an effort to show the issue to the countries in and understandable way (showing simultaneously EMP, SAL and the calculated SELF).
...
We have removed many elements of the full report which have been added to basic report. When we arrive at this step we should have a complete transmission that satisfies the minimum requirements and we start a more analytic validation process. The report will provide some, mainly graphical, basic elements to detect in which areas we should, if needed, investigate further. It creates a day stamped html file in `report/` when executing. There is an argument to choose the height and width of the charts (8x8) works for most countries but fro some particular cases (DE, FR, IT) a bigger number would be more appropriate or smaller (SI, LV, EE)
...
```{r, echo=FALSE}
library(ggiraph)
source(here("utils","theme_report.R"))
...
The other tables show the share of the regions in the country.

```{r,echo=FALSE,warning=FALSE,message=FALSE,fig.height=12, fig.width=14}
...
temp<- t1200 %>% 
 ...
 x <- girafe(ggobj = gg, width_svg = 12, height_svg = 12,
             options = list(
               opts_hover_inv(css = "opacity:0.1;"),
               opts_hover(css = "stroke-width:2;")))
...
emp_regacc <-list.files(path= here("data","csv"),
                pattern= glob2rx(paste0("*",country_sel,"*")),
                full.names=TRUE) %>% 
  as_tibble() %>% 
  mutate(date=map(value,file.mtime)) %>% 
...
emp_lfs <- readRDS(here("data","eurobase","emp_lfs.rds")) %>% 
  rename(ref_area=geo,
         time_period=time)
...
A set of three shiny apps should help us in investigating further some specific topics (revisions, outliers, last year data) and get some information for some analytical ratios (combination of variables). A brief look at t1001 is recommended to check the plausibility of the latest year and to the indicators that will later show up in publications (D1 per hour worked and person, income per capita, productivity, etc).

It is recommended to use Chrome (or alternatively Firefox) and do not forget to stop the shiny app when finished. Do not forget to run first the code defining the function and only later run the function.

This year we have introduced some improvements in the reactivity for the application with NUTS 3 data (t1002_1200) and a default scatterplot view show levels and growth rates that can help us in the outlier detection.
...
Another Matis script extracts data for EMP and B1G for the last four years and sends them to the NQR Fame domain in order to compare them with main aggregates.
...
The charts should be preferably extracted from the shiny apps or prepared using the `ra*` functions of `regacc`. A copy of the charts and the code for the charts should be kept in a working folder (no need to copy them on the server).
...
```{r,echo=FALSE}
fs::dir_tree(here(),recurse = FALSE)
```
