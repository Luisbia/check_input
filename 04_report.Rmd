...
files<-list.files(path="data/denodo",
...
# logo for the report
knitr::include_graphics("eurostat_logo.svg")
```
...
completeness<- function (input) {
  
  temp <- files %>% 
    filter(type=="T") %>% # filter only T
    left_join(input,.) # join with the respective theoretical table
  
  inc<- temp %>% 
  group_by(country) %>% # group by country
  summarise(reported=sum(!is.na(obs_value))) %>% # check how many are empty
  pull(reported) # extract as value
  
  tot<-temp %>% 
  group_by(country) %>% 
  summarise(total=n()) %>% 
    pull(total) # count all
  
  temp<-round(inc*100/tot,2) #calculate the ratio
  return (temp)
   }

t1001c<-completeness (t1001)
t1002c<-completeness (t1002)
t1200c<-completeness (t1200)
t1300c<-completeness (t1300)

coverage<- tibble("t1001"=t1001c,"t1002"=t1002c,"t1200"=t1200c,"t1300"=t1300c)
 
t1001c<-donut_plot(value=t1001c/100, title="Table 1001")
t1002c<-donut_plot(value=t1002c/100, title="Table 1002")
t1200c<-donut_plot(value=t1200c/100, title="Table 1200")
t1300c<-donut_plot(value=t1300c/100, title="Table 1300")
